...
def upgrade():
    ...
    conn.execute(sa.text(f"""
        CREATE TABLE IF NOT EXISTS {_s}teamguard_members (
            id                    SERIAL PRIMARY KEY,
            owner_user_id         INTEGER NOT NULL
                REFERENCES {_s}users(id) ON DELETE CASCADE,
    ...
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_members_owner_user_id
            ON {_s}teamguard_members (owner_user_id)
    """))

    # ── teamguard_password_policy ────────────────────────────────
    conn.execute(sa.text(f"""
        CREATE TABLE IF NOT EXISTS {_s}teamguard_password_policy (
            id                   SERIAL PRIMARY KEY,
    ...
    conn.execute(sa.text(f"""
        CREATE TABLE IF NOT EXISTS {_s}teamguard_password_assignments (
            id              SERIAL PRIMARY KEY,
            member_id       INTEGER NOT NULL
                REFERENCES {_s}teamguard_members(id) ON DELETE CASCADE,
    ...
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_password_assignments_member_id
            ON {_s}teamguard_password_assignments (member_id)
    """))

    # ── teamguard_security_events ────────────────────────────────
    conn.execute(sa.text(f"""
        CREATE TABLE IF NOT EXISTS {_s}teamguard_security_events (
            id           SERIAL PRIMARY KEY,
            member_id    INTEGER NOT NULL
                REFERENCES {_s}teamguard_members(id) ON DELETE CASCADE,
            owner_user_id INTEGER NOT NULL,
    ...
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_security_events_member_id
            ON {_s}teamguard_security_events (member_id)
    """))
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_security_events_owner_user_id
            ON {_s}teamguard_security_events (owner_user_id)
    """))
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_security_events_created_at
            ON {_s}teamguard_security_events (created_at)
    """))

    # ── teamguard_phishing_tests ─────────────────────────────────
    conn.execute(sa.text(f"""
        CREATE TABLE IF NOT EXISTS {_s}teamguard_phishing_tests (
            id                      SERIAL PRIMARY KEY,
            owner_user_id           INTEGER NOT NULL
                REFERENCES {_s}users(id) ON DELETE CASCADE,
    ...
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_phishing_tests_owner_user_id
            ON {_s}teamguard_phishing_tests (owner_user_id)
    """))
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_phishing_tests_tracking_token
            ON {_s}teamguard_phishing_tests (tracking_token)
    """))

    # ── teamguard_phishing_clicks ────────────────────────────────
    conn.execute(sa.text(f"""
        CREATE TABLE IF NOT EXISTS {_s}teamguard_phishing_clicks (
            id                  SERIAL PRIMARY KEY,
            test_id             INTEGER NOT NULL
                REFERENCES {_s}teamguard_phishing_tests(id) ON DELETE CASCADE,
            member_id           INTEGER NOT NULL
                REFERENCES {_s}teamguard_members(id) ON DELETE CASCADE,
    ...
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_phishing_clicks_test_id
            ON {_s}teamguard_phishing_clicks (test_id)
    """))
    conn.execute(sa.text(f"""
        CREATE INDEX IF NOT EXISTS ix_teamguard_phishing_clicks_member_id
            ON {_s}teamguard_phishing_clicks (member_id)
    """))
...
def downgrade():
    ...
    for tbl in [
        ...
        conn.execute(sa.text(f"DROP TABLE IF EXISTS {_s}{tbl} CASCADE"))
