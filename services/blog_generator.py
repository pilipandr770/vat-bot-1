...
Claude generiert ~900-1200 Wörter Fließtext + HTML-Markup.
"""
...
def _generate_article_with_claude(topic: Dict, rss_context: Optional[Dict]) -> Optional[str]:
    """Call Claude (Anthropic) to generate a German SEO article in HTML."""
    api_key = os.environ.get('ANTHROPIC_API_KEY')
    if not api_key:
        logger.warning("ANTHROPIC_API_KEY nicht gesetzt — Blog-Generierung übersprungen")
        return None
...
"
    ...
    system_prompt = (
        "Du bist ein SEO-Experte und Content-Autor für B2B-Sicherheit und Compliance "
        "im deutschen Markt. Schreibe professionelle, informative Artikel für "
        "Entscheidungsträger (GeschäftsführerInnen, Compliance-Manager, CFOs). "
        "Stil: sachlich-professionell, verständlich, keine Werbetexte."
    )

    user_prompt = f"""Schreibe einen SEO-Blog-Artikel auf Deutsch.
...
- Struktur: Einleitung → 4-5 Abschnitte mit <h2> → Fazit
- Keyword in Einleitung, ≥2 <h2>-Überschriften und im Fazit
- Konkrete Beispiele und handlungsrelevante Tipps
- Max. 2-3 natürliche Erwähnungen von VAT Verifizierung (https://vat-verifizierung.de)
- Am Ende: FAQ-Block mit 3 Fragen als <div class="faq-section"><h3>Häufig gestellte Fragen</h3><dl><dt>...</dt><dd>...</dd></dl></div>
- Nur HTML-Body zurückgeben (kein <html>, <head>, <body>)
- Bootstrap-Markup: <h2>, <h3>, <p>, <ul>, <li>, <strong>"""
    ...
    try:
        ...
        logger.error(f"Claude Blog-Generierung fehlgeschlagen: {e}", exc_info=True)
        return None
...
def generate_daily_blog_post(app, force: bool = False) -> bool:
    ...
    with app.app_context():
        ...
        # 3. Generate content
        body_html = _generate_article_with_claude(topic, rss_context)
        if not body_html:
            logger.warning("Blog: Artikel-Generierung fehlgeschlagen (kein Claude Inhalt)")
            return False
