...
## ⚠️ ВАЖНО: Кто получает OAuth ключи?

### 🔐 Разработчик (вы) - ОДИН РАЗ:
1. ✅ Создаете OAuth приложение в **Google Cloud Console**
2. ✅ Создаете OAuth приложение в **Azure Portal**
3. ✅ Добавляете 6 переменных окружения в **Render.com**
4. ✅ Деплоите код с OAuth routes

### 👤 Обычный пользователь - ПРОСТОЙ ПРОЦЕСС:
```
1. Заходит на /mailguard/accounts
2. Нажимает "Подключить Gmail" 
3. Видит официальный Google popup: "Разрешить доступ?"
4. Нажимает "Разрешить"
5. ✅ Готово! Почта подключена
```

**Пользователь НЕ работает с:**
- ❌ Client ID / Client Secret
- ❌ Google Cloud Console / Azure Portal
- ❌ API настройками
- ❌ Техническими деталями

**Он просто разрешает доступ через официальный OAuth popup!**
